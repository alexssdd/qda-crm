<?php

use yii\helpers\Url;
use yii\helpers\Html;

/** @var $data [] */

?>
<?php if ($data['count']) : ?>
    <div class="chart-circle">
        <div class="chart-circle__info">
            <div class="chart-circle__left">
                <div class="chart-circle__value"><?= Html::encode((string) $data['count']) ?></div>
            </div>
            <div class="cart-circle__right">
                <div class="chart-circle__legends">
                    <div class="chart-circle__legend">
                        <div class="chart-circle__legend-value"><?= Html::encode((string) $data['count_10']) ?></div>
                        <div class="chart-circle__legend-key">более</div>
                        <div class="chart-circle__legend-time">10 минут</div>
                    </div>
                    <div class="chart-circle__legend">
                        <div class="chart-circle__legend-value"><?= Html::encode((string) $data['count_20']) ?></div>
                        <div class="chart-circle__legend-key">более</div>
                        <div class="chart-circle__legend-time">20 минут</div>
                    </div>
                </div>
            </div>
        </div>
        <div class="chart-circle__buttons">
            <?php foreach ($data['orders'] as $order) : ?>
                <a target="_blank" href="<?= Url::to(['/order/index', 'id' => $order['id']]) ?>" class="chart-circle__button">
                    <?= Html::encode((string) $order['number']) ?>
                </a>
            <?php endforeach; ?>
        </div>
    </div>
<?php else : ?>
    <div class="chart-success">
        <i class="icon-check chart-success__icon"></i>
    </div>
<?php endif; ?>
